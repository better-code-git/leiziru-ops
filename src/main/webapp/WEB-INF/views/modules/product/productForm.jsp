...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/product/product/form?id=${product.id}">房源管理<shiro:hasPermission name="product:product:edit">${not empty product.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="product:product:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="product" action="" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">	
				<form:input path="productName" htmlEscape="false" maxlength="255" class="input-medium required"/>
				<label>月租金：</label>
				<form:input path="productPrice" htmlEscape="false" maxlength="255" class="input-medium required"/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">	
				<form:select path="productType" style="width: 100px;padding:0;"   class="form-control m-b">
					<form:option value="" label="请选择"/>
					<form:options items="${fns:getDictList('lzr_product_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
		...
		<div class="control-group">
			...
			<div class="controls">	
				<form:select path="productProvinceId" style="width: 100px;height: 25px;padding:0;"   class="form-control m-b">
					<form:option value="" label="请选择"/>
				...
				<form:select path="productDistrictId" style="width: 100px;height: 25px;padding:0;"   class="form-control m-b">
					<form:option value="" label="请选择"/>
			...
			<div class="controls">	
				...
				<label>朝向：</label>
					<form:select path="productOrientation" style="width: 100px;padding:0;"   class="form-control m-b">
						<form:option value="" label="请选择"/>
						<form:options items="${fns:getDictList('lzr_product_orientation')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
					</form:select>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">	
				<form:select path="decorateType" style="width: 100px;padding:0;"   class="form-control m-b">
					<form:option value="" label="请选择"/>
					<form:options items="${fns:getDictList('lzr_decorate_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<form:textarea id="productIntroduce" htmlEscape="true" path="productIntroduce" class="input-xxlarge required" />
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">	
				...
				<c:forEach items="${product.productAssortList}" var="productAssort" varStatus="status">
				  <tr><td>
						...
						<form:input path="productAssortList[${status.index}].roomName" htmlEscape="false" class="input-medium"/>
						<form:input path="productAssortList[${status.index}].roomArea" htmlEscape="false" class="input-medium"/>
						<form:radiobuttons path="productAssortList[${status.index}].assort" items="${fns:getDictList('lzr_product_assort')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
			     	    <input id="addProductAssort"  onclick="delProductAssort(this)"  class="btn" type="button" value="删除"/>&nbsp;</label>
			      </td></tr>
		...
		<div class="control-group">
	    	...
	    	<div class="controls">	
		    	<form:hidden path="productProfileUrl"/>
				<input type="hidden" name="productProfileUrlOld" id="productProfileUrlOld" value="${product.productProfileUrl}"/>
				<table:addPic showUrl="productProfileUrl" imageUrl="${product.productProfileUrl}" fileDir="product" imagePreview="viewImg" imgFileData="imgFileData"></table:addPic>
			</div>
		</div>
		<div class="control-group">
		    ...
		    <c:forEach items="${product.productPicList}" var="productPic" varStatus="status" >
			   <td >
				    ...
				    <input type="hidden" name="productPicList[${status.index}].picUrlOld" id="productPicList[${status.index}].picUrlOld" value="${productPic.picUrl}"/>
					<form:input path="productPicList[${status.index}].picDesc" class="form-control" maxlength="64" cssStyle="width:80px;"/>
				    <form:hidden path="productPicList[${status.index}].picUrl"  id="productPicList[${status.index}].picUrl"/>
				    <table:addPic showUrl="productPicList[${status.index}].picUrl" imageUrl="${productPic.picUrl}" fileDir="product" imagePreview="viewImg_${status.index}" imgFileData="productPicList[${status.index}].imgFileData"></table:addPic>
			   </td>
			</c:forEach>
			</tr>
			</table>
