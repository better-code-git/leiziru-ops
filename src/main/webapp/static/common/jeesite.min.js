...
/**
 * 打开对话框(添加修改)新版本，陆续改造中
 **/
function openDialogPro(title,url,urlOp,width,height){
	...
	top.layer.open({
	    ...
	    yes: function(index, layero){
	         ...
	         var validateForm=iframeWin.contentWindow.validateForm;
        	 if(doValidateForm(validateForm)){
        		 doSubmitForAjax(urlOp,inputForm,index);
        	 }
		  },
 ...
 *  inputForm 逻辑规则校验
 */
function doValidateForm(validateForm){
	   var rtn=false;
...
/**
 *  inputForm ajax 提交到后台  
 *  成功并刷新父列表页，不成功提示报错，停留在当前页
 */
function doSubmitForAjax(urlOp,inputForm,index) {
	var getTimestamp=new Date().getTime();
	if(urlOp.indexOf("?")>-1){
		urlOp=urlOp+"&timestamp="+getTimestamp;
      }else{
    	urlOp=urlOp+"?timestamp="+getTimestamp;
      }
 ...
 *  不做刷新页面
 */
function doSubmitForAjaxNoRefresh(urlOp,inputForm) {
	var getTimestamp=new Date().getTime();
	if(urlOp.indexOf("?")>-1){
		urlOp=urlOp+"&timestamp="+getTimestamp;
      }else{
 ...
 *不走重定向的公共方法。
 */
function updateOp(href){
	var getTimestamp=new Date().getTime();
	if(href.indexOf("?")>-1){
		href=href+"&timestamp="+getTimestamp;
      }else{
    	href=href+"?timestamp="+getTimestamp;
      }
	$.ajax({
...
function reset(){//重置，页码清零
	$("#pageNo").val(0);
	$("#searchForm ul.ul-form input").val("");
	$("#searchForm ul.ul-form select").val("");
	$("#searchForm").submit();
    return false;
}
